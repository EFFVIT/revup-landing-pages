    ...
    <div id="consult">
      ...
      <footer className="consult-footer">
        ...
        <nav className="cf-links">
          ...
          {/* DIVERGENCE FROM THE SHARED ENGINE, 2026-08-17, and it should be
              propagated back to the other seven repos so this file returns to
              byte-identical. Every other client on the fleet carries a
              /cookie-policy route inside its own repo, so cookieUrl was always
              a string and never needed a guard. Rev Up has no cookie policy
              page at all — /cookie-policy/ is a 404 — and its cookie
              disclosure lives as a "Cookies and analytics" section inside the
              privacy policy instead. Rendering the link unguarded pointed at a
              404, which the comment below already names as a Google Ads
              destination problem rather than a cosmetic one, and it also fails
              the build because cookieUrl is typed string | null here.
              The guard matches the one termsUrl already has. It cannot hide
              anything: a client that HAS a cookie policy still renders it. */}
          {CONSULT.cookieUrl && <a href={CONSULT.cookieUrl}>Cookie Policy</a>}
